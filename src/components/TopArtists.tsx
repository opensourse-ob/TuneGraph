...
const TopArtists = () => {
  ...
  return (
    <div className="text-black-500 bg-emerald-500 border-black p-1 m-2">
      <Tabs value={timeRange} onValueChange={handleTimeRangeChange}>
        <TabsList className="grid w-full grid-cols-3 bg-black border-emerald-500">
          {timeRanges.map(range => (
            <TabsTrigger
              key={range.value}
              value={range.value}
              className="text-emerald-400 data-[state=active]:bg-emerald-500/40"
            >
              {range.label}
            </TabsTrigger>
          ))}
        ...
        {timeRanges.map(range => (
          <TabsContent key={range.value} value={range.value} className="mt-4">
            <Card className="border-emerald-500 bg-emerald-500 text-black">
              <CardHeader>
                <CardTitle className="flex justify-center text-emerald-100 text-2xl">
                  Top Artists
                </CardTitle>
                <CardDescription className="flex justify-center text-emerald-900">
