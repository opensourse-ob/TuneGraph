...
router.get("/login", (req: Request, res: Response) => {
  ...
  pendingStates.add(state); //store random string on server

  const scope = SCOPES.length > 0 ? SCOPES.join(" ") : undefined; //accses wich app gets from user it has to be string
  ...
  // Add scope to params only if scopes are defined (optional parameter per Spotify docs)
  if (scope) {
    params.scope = scope;
  }
  ...
  //convert params into string with & %3A...
  const queryString = new URLSearchParams(params).toString();
  res.redirect(`https://accounts.spotify.com/authorize?${queryString}`); //makes the whole url string
});
...
router.get("/callback", async (req: Request, res: Response) => {
  //read request from spotify and parse
  ...
  //------------verify info wich we got from spotify-----
  ...
  //we got code from spotify and now we need to exchange this code to tokens(acsess and refresh)
  try {
    ...
    const tokenResponse = await fetch(
      "https://accounts.spotify.com/api/token",
      {
        ...
        headers: {
          "Content-Type": "application/x-www-form-urlencoded", //send form format, spotify's request
          Authorization: `Basic ${Buffer.from(`${SPOTIFY_CLIENT_ID}:${SPOTIFY_CLIENT_SECRET}`).toString("base64")}`,
        }, //app's credentions converts into random string for saving
        body: new URLSearchParams({
          grant_type: "authorization_code",
          code: code, //code from spotify
          redirect_uri: SPOTIFY_REDIRECT_URI,
        }),
      }
    );
    //if spotify returns error
    if (!tokenResponse.ok) {
    ...
    const tokenData = (await tokenResponse.json()) as SpotifyTokenResponse; //convert into obj

    //get token and save inside cookie
    ...
    //save refresh token inside cookie - needed for users not to log in every hours
...
router.post("/refresh", async (req: Request, res: Response) => {
  ...
  //request to spotofy for getting new accsess token
  try {
    const tokenResponse = await fetch(
      "https://accounts.spotify.com/api/token",
      {
        ...
        body: new URLSearchParams({
          ...
          refresh_token: refreshToken, //send our refresh token
        }),
      }
    );
