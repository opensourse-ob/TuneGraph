...
router.get('/top-artists', requireAuth, async (req: Request, res: Response) => {
    try {
        ...
        const data = await spotifyApiRequest(accessToken, `/me/top/artists?time_range=${timeRange}&limit=${limit}`) as SpotifyTopArtistsResponse;

        // Shape object to match UI needs
        const topArtists = data.items.map((artist: SpotifyArtist, index: number) => ({
...
router.get('/top-songs', requireAuth, async (req: Request, res: Response) => {
    try{
        ...
        const data = await spotifyApiRequest(accessToken, `/me/top/tracks?time_range=${timeRange}&limit=${limit}`) as SpotifyTopTracksResponse;
...
router.get('/top-genres', requireAuth, async(req: Request, res: Response) => {
    try{
        ...
        const data = await spotifyApiRequest(accessToken, `/me/top/artists?time_range=${timeRange}&limit=${limit || 50}`) as SpotifyTopArtistsResponse;
